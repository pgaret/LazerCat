...
public class MouseLook : MonoBehaviour
{
    ...
    {
        if (GetComponent<Person>().onCube == false)
        {
            if (angle <= -360F)
                angle = -360F;
            if (angle >= 360F)
        ...
        if (GetComponent<Person>().onCube == true)
        {
            if (angle <= 0F + GetComponent<Person>().cube.rotation.y)
                angle = 0F + GetComponent<Person>().cube.rotation.y;
            if (angle >= 180F + GetComponent<Person>().cube.rotation.y)
                angle = 180F + GetComponent<Person>().cube.rotation.y;
            return Mathf.Clamp(angle, min, max);
        }
        return angle;
